(function($,window,document,undefined){var pluginName='autoHidingNavbar',$window=$(window),$document=$(document),_scrollThrottleTimer=null,_resizeThrottleTimer=null,_throttleDelay=70,_lastScrollHandlerRun=0,_previousScrollTop=null,_windowHeight=$window.height(),_visible=!0,_hideOffset,defaults={disableAutohide:!1,showOnUpscroll:!0,showOnBottom:!0,hideOffset:'auto',animationDuration:200,navbarOffset:0};function AutoHidingNavbar(element,options){this.element=$(element);this.settings=$.extend({},defaults,options);this._defaults=defaults;this._name=pluginName;this.init()}
function hide(autoHidingNavbar){if(!_visible){return}
autoHidingNavbar.element.addClass('navbar-hidden').animate({top:-1*parseInt(autoHidingNavbar.element.css('height'),10)+autoHidingNavbar.settings.navbarOffset},{queue:!1,duration:autoHidingNavbar.settings.animationDuration});try{$('.dropdown.open .dropdown-toggle, .dropdown.show .dropdown-toggle',autoHidingNavbar.element).dropdown('toggle')}
catch(e){}
_visible=!1;autoHidingNavbar.element.trigger('hide.autoHidingNavbar')}
function show(autoHidingNavbar){if(_visible){return}
autoHidingNavbar.element.removeClass('navbar-hidden').animate({top:0},{queue:!1,duration:autoHidingNavbar.settings.animationDuration});_visible=!0;autoHidingNavbar.element.trigger('show.autoHidingNavbar')}
...
else{clearTimeout(_scrollThrottleTimer);_scrollThrottleTimer=setTimeout(function(){scrollHandler(autoHidingNavbar)},_throttleDelay)}});$window.on('resize.'+pluginName,function(){clearTimeout(_resizeThrottleTimer);_resizeThrottleTimer=setTimeout(function(){_windowHeight=$window.height()},_throttleDelay)})}
